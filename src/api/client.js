...
import { getUpstreamStatus, readUpstreamErrorBody, isCallerDoesNotHavePermission, getValidationRequiredDetails } from './upstreamError.js';
...
import { parseQuotaSummary, quotaSummaryUrlFromModelsUrl } from '../auth/quotaSummary.js';
...
function buildHeaders(token, hostOverride = null) {
	return {
		...
		// Let the HTTP client choose exactly one framing strategy. Generation
		// requests set Content-Length below; combining it with chunked transfer
		// encoding causes Google upstreams to terminate the connection.
...
async function handleApiError(error, token, dumpId = null) {
  ...
  if (status === 403) {
    ...
    const validation = getValidationRequiredDetails(errorBody);
    if (validation) {
      await tokenManager.disableToken(token, validation);
      throw createApiError(`该账号需要 Google 验证，已自动禁用。错误详情: ${errorBody}`, status, errorBody);
    }
  ...
  try {
    ...
    await withUpstreamFallback(async (candidate) => {
      ...
      const state = {
        ...
        sessionId: requestBody._signatureSessionId || token.signatureSessionId || requestBody.request?.sessionId,
    ...
    await tokenManager.markValidationSuccessful(token);
...
export async function getModelsWithQuotas(token) {
  ...
  Object.entries(data.models || {}).forEach(([modelId, modelData]) => {
    if (modelData.quotaInfo) {
      quotas[modelId] = {
        r: typeof modelData.quotaInfo.remainingFraction === 'number'
          ? modelData.quotaInfo.remainingFraction
          : null,
        known: typeof modelData.quotaInfo.remainingFraction === 'number',
...
export async function fetchQuotaSummary(token) {
  const url = quotaSummaryUrlFromModelsUrl(config.api.modelsUrl);
  if (!url || url === config.api.modelsUrl) return null;

  try {
    const { data } = await requesterManager.fetch(url, {
      method: 'POST',
      headers: buildHeaders(token),
      body: token.projectId ? { project: token.projectId } : {}
    });
    return parseQuotaSummary(data);
  } catch (error) {
    logger.warn('拉取周额度失败:', error.message);
    return null;
  }
}

export async function refreshTokenQuotaCache(tokenId, token) {
  ...
  if (weekly) quotaManager.updateQuotaSummary(tokenId, weekly);
  return quotas;
}
...
export async function generateAssistantResponseNoStream(requestBody, token) {
  ...
  const sessionId = requestBody._signatureSessionId || token.signatureSessionId || requestBody.request?.sessionId;
