...
import { generateAssistantResponse, generateAssistantResponseNoStream, getAvailableModels, refreshTokenQuotaCache } from '../../api/client.js';
...
export const handleGeminiRequest = async (req, res, modelName, isStream) => {
  ...
  try {
    const body = req.body || {};
    const sessionKey = getRequestSessionKey(req, 'gemini', modelName, body);
    ...
    if (!await applyTokenState(await tokenManager.getToken(modelName, sessionKey))) {
    ...
    const createRetryOptions = (prefix) => ({
      ...
      onBeforeRetry: async ({ previousTokenId }) => {
        const nextToken = await tokenManager.getTokenForRetry(modelName, previousTokenId, sessionKey);
