...
export const handleClaudeRequest = async (req, res, isStream) => {
  ...
  const sessionKey = getRequestSessionKey(req, 'claude', model, body);
  ...
  try {
    ...
    if (!await applyTokenState(await tokenManager.getToken(model, sessionKey))) {
    ...
    const createRetryOptions = (prefix) => ({
      ...
      onBeforeRetry: async ({ previousTokenId }) => {
        const nextToken = await tokenManager.getTokenForRetry(model, previousTokenId, sessionKey);
