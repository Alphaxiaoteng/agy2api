...
import { generateAssistantResponse, generateAssistantResponseNoStream, refreshTokenQuotaCache } from '../../api/client.js';
import { generateRequestBody, modelMapping, prepareImageRequest } from '../../utils/utils.js';
...
import { getRequestSessionKey, scopeTokenToRequest } from '../../utils/requestSession.js';
import {
  createResponseMeta,
  setStreamHeaders,
  createHeartbeat,
...
export const handleOpenAIRequest = async (req, res) => {
  ...
  const sessionKey = getRequestSessionKey(req, 'openai', model, body);
  ...
  try {
    ...
    const upstreamModel = modelMapping(model);
    ...
    const applyTokenState = async (nextToken) => {
      ...
      token = scopeTokenToRequest(nextToken, sessionKey);
    ...
    if (!await applyTokenState(await tokenManager.getToken(upstreamModel, sessionKey))) {
    ...
    const createRetryOptions = (prefix) => ({
      loggerPrefix: prefix,
      onAttempt: () => tokenManager.recordRequest(token, upstreamModel),
      getTokenId: () => tokenId,
      ...
      onBeforeRetry: async ({ previousTokenId }) => {
        const nextToken = await tokenManager.getTokenForRetry(upstreamModel, previousTokenId, sessionKey);
