...
export async function with429Retry(fn, maxRetries, options = {}, legacyOnAttempt = null) {
  ...
  while (true) {
    try {
      ...
      const canRotateToken = typeof retryOptions.onBeforeRetry === 'function';
      const authFailure = status === 401 && canRotateToken;
      if (status !== 429 && status !== 503 && !authFailure) {
        throw error;
      }
      ...
      if (authFailure) {
        const tokenManager = retryOptions.tokenManager;
        const token = getCurrentToken(retryOptions);
        if (tokenManager && token && typeof tokenManager.disableToken === 'function') {
          try {
            await tokenManager.disableToken(token, { reason: 'AUTHENTICATION_FAILED' });
          } catch (disableError) {
            logger.warn(`${loggerPrefix}401 账号隔离失败: ${disableError.message}`);
          }
        }
        ...
        logger.warn(`${loggerPrefix}收到上游 401，已隔离当前账号，立即切换下一个账号（第 ${nextAttempt} 次）`);
        const prepared = await prepareNextAttempt(retryOptions, {
          ...
          previousTokenId,
          modelId,
          longQuotaCooldown: false
        });

        if (!prepared) {
          logger.warn(`${loggerPrefix}切换账号失败，没有其他可用 token`);
          throw error;
        }
