...
function getApiKey() {
  ...
  // 显式配置（含空串）一律以配置为准：`.env` 里写 `API_KEY=` 即「这台不启用鉴权」——
  // 本地回环部署的常态（服务绑 127.0.0.1，见 config.server.host）。空值走这个分支而不是
  // 掉进下面的随机生成，否则用户永远关不掉鉴权。未配置（undefined）时才自动生成随机密钥，
  // 免得把服务裸奔到公网的人没有门槛。
  if (apiKey !== undefined) {
    return apiKey;
  }
...
export function buildConfig(jsonConfig, upstreamCfg = {}) {
  ...
  return {
    server: {
      port: Number.parseInt(process.env.AGY_SERVER_PORT || '', 10) || jsonConfig.server?.port || DEFAULT_SERVER_PORT,
      host: process.env.AGY_SERVER_HOST || jsonConfig.server?.host || DEFAULT_SERVER_HOST,
    ...
    rotation: {
      ...
      priorityAccountIds: normalizePriorityAccountIds(
        jsonConfig.rotation?.priorityAccountIds ??
    ...
    // 模型目录：从 OpenCode 配置动态汇总，路径可通过 env / config 覆盖，不写死端口
    catalog: {
      opencodeConfigPath:
        process.env.OPENCODE_CONFIG ||
        process.env.OPENCODE_CONFIG_PATH ||
        jsonConfig.catalog?.opencodeConfigPath ||
        ''
    },
    // 图片上游：优先 config/env，缺省时由 OpenCode provider.baseURL 推导
    imageUpstream: {
      codex: {
        url: jsonConfig.imageUpstream?.codex?.url || process.env.CODEX_IMAGE_URL || '',
        apiKey: jsonConfig.imageUpstream?.codex?.apiKey || process.env.CODEX_API_KEY || ''
      },
      doubao: {
        url: jsonConfig.imageUpstream?.doubao?.url || process.env.DOUBAO_IMAGE_URL || '',
        apiKey: jsonConfig.imageUpstream?.doubao?.apiKey || process.env.DOUBAO_API_KEY || ''
      }
    },
    api: apiConfig,
    defaults: {
    ...
    codexUpstream: {
      enabled: jsonConfig.codexUpstream?.enabled !== false,
      baseURL: jsonConfig.codexUpstream?.baseURL || process.env.CODEX_LOCAL_BASE_URL || 'http://127.0.0.1:61175/v1',
      configPath: jsonConfig.codexUpstream?.configPath || path.join(os.homedir(), '.antigravity_cockpit/codex_local_access.json'),
      apiKey: process.env.CODEX_LOCAL_API_KEY || jsonConfig.codexUpstream?.apiKey || '',
      modelListTtlMs: Number.isFinite(jsonConfig.codexUpstream?.modelListTtlMs)
        ? jsonConfig.codexUpstream.modelListTtlMs
        : 300_000,
      validateModelOnChat: jsonConfig.codexUpstream?.validateModelOnChat === true
    },
