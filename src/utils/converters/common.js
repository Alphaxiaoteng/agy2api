...
import { getThoughtSignatureForModel, getToolSignatureForModel, sanitizeToolName, modelMapping, isEnableThinking, generateGenerationConfig, GEMINI_TOOL_SIGNATURE, CLAUDE_TOOL_SIGNATURE, GEMINI_THOUGHT_SIGNATURE, CLAUDE_THOUGHT_SIGNATURE } from '../utils.js';
...
export function getSignatureContext(sessionId, actualModelName, hasTools = false) {
  ...
  // 图片模型：不注入任何 thoughtSignature，不读缓存，不用 fallback
  if (isImage) {
    return {
  ...
  if (cachedEntry) {
    ...
    // 即使 useFallbackSignature 为 false，依然提供默认模型签名备用（供思考模型 toolCalls / functionCalls 兜底使用）
    toolSignature = getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE;
    reasoningSignature = getThoughtSignatureForModel(actualModelName) || GEMINI_THOUGHT_SIGNATURE;
  }
...
export function findFunctionNameById(toolCallId, antigravityMessages) {
  if (!toolCallId || !Array.isArray(antigravityMessages)) return '';
  for (let i = antigravityMessages.length - 1; i >= 0; i--) {
    if (antigravityMessages[i].role === 'model') {
      const parts = antigravityMessages[i].parts;
      for (const part of parts) {
        if (part.functionCall && (part.functionCall.id === toolCallId || String(part.functionCall.id) === String(toolCallId))) {
 ...
 * @param {string} actualModelName - 实际模型名称（可选，用于在思考模型下自动兜底补签名）
 * @returns {Object} 函数调用 part
 */
export function createFunctionCallPart(id, name, args, signature, actualModelName) {
  let finalSignature = signature;
  if (actualModelName && isImageModel(actualModelName)) {
    finalSignature = null;
  } else if (!finalSignature && actualModelName) {
    let enableThinking = false;
    try {
      if (typeof isEnableThinking === 'function') {
        enableThinking = isEnableThinking(actualModelName);
      }
    } catch (e) {}
    if (enableThinking) {
      if (typeof getToolSignatureForModel === 'function') {
        finalSignature = getToolSignatureForModel(actualModelName);
      }
      if (!finalSignature) {
        finalSignature = (typeof GEMINI_TOOL_SIGNATURE !== 'undefined') ? GEMINI_TOOL_SIGNATURE : 'bWVzc2FnZV9pZA==';
      }
    }
  }
  ...
  if (finalSignature) {
    part.thoughtSignature = finalSignature;
  }
  return part;
}
