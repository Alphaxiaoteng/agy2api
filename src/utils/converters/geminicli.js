      ...
      if (msg.tool_calls && Array.isArray(msg.tool_calls)) {
        for (const toolCall of msg.tool_calls) {
          if (toolCall.type === 'function') {
            ...
            const tcSignature = toolCall.thoughtSignature || toolCall.thought_signature || toolCall.signature || toolCall.function?.thoughtSignature || toolCall.function?.thought_signature || toolCall.extra_content?.thought_signature;
            // 工具调用始终需要签名（无论是否启用思考模式）
            const signature = tcSignature || toolSignature || reasoningSignature || getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE || SKIP_THOUGHT_SIGNATURE_VALIDATOR;
            parts.push(createFunctionCallPart(toolCall.id, safeName, args, signature, actualModelName));
          }
        }
      }
...
function processGeminiModelThoughts(content, reasoningSignature, reasoningContent, toolSignature, toolContent, enableThinking, actualModelName) {
  ...
  if (Array.isArray(parts)) {
    for (const part of parts) {
      if (part && part.thought_signature && !part.thoughtSignature) {
  ...
  if (!enableThinking) {
    if (isImageModel(actualModelName)) return;
  ...
  } else if (thoughtIndex === -1 && fallbackSig && config.useFallbackSignature) {
    // 只有在开启兜底签名或有缓存签名时才添加 thought part
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if ((!part.thoughtSignature) && (part.functionCall || part.inlineData)) {
      ...
      const partFallback = part.functionCall
        ? (toolSignature || reasoningSignature || getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE || SKIP_THOUGHT_SIGNATURE_VALIDATOR)
        : (reasoningSignature || toolSignature || getThoughtSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE || SKIP_THOUGHT_SIGNATURE_VALIDATOR);
  ...
  if (enableThinking && request.contents && Array.isArray(request.contents)) {
    ...
    for (const content of request.contents) {
      if (content.role === 'model' && content.parts && Array.isArray(content.parts)) {
        processGeminiModelThoughts(content, reasoningSignature, reasoningContent, toolSignature, toolContent, enableThinking, actualModelName);
      }
    }
  }
      ...
      if (typeof msg.content === 'string') {
        ...
        for (const item of msg.content) {
          if (item.type === 'text') {
            ...
            const itemSig = item.signature || item.thoughtSignature || item.thought_signature || item.extra_content?.thought_signature;
            // 工具调用始终需要签名（无论是否启用思考模式）
            const signature = itemSig || toolSignature || messageSignature || reasoningSignature || getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE || SKIP_THOUGHT_SIGNATURE_VALIDATOR;
            toolCalls.push(createFunctionCallPart(item.id, safeName, item.input || {}, signature, actualModelName));
          }
        }
      }
