...
import { getSignatureContext, createThoughtPart, modelMapping, isEnableThinking, buildSystemInstruction, getThoughtSignatureForModel, getToolSignatureForModel, GEMINI_TOOL_SIGNATURE } from './common.js';
 ...
 */
function processModelThoughts(content, reasoningSignature, reasoningContent, toolSignature, toolContent, enableThinking, actualModelName) {
  ...
  if (thoughtIndex !== -1 && signatureIndex !== -1) {
    ...
    // 只有在开启兜底签名或有缓存签名时才添加 thought part，避免 API 报错
    // 使用与签名绑定的缓存内容
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if ((!part.thoughtSignature) && (part.functionCall || part.inlineData)) {
      ...
      // functionCall 更倾向 toolSignature；inlineData 更��向 reasoningSignature
      const partFallback = part.functionCall
        ? (toolSignature || reasoningSignature || getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE)
        : (reasoningSignature || toolSignature || getThoughtSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE);
...
export function generateGeminiRequestBody(geminiBody, modelName, token) {
  ...
  if (request.contents && Array.isArray(request.contents)) {
    ...
    if (request.tools && Array.isArray(request.tools)) {
      request.tools = convertGeminiToolsToAntigravity(request.tools, signatureSessionId, actualModelName);
    }

    const hasTools = request.tools && request.tools.length > 0;
    const { reasoningSignature, reasoningContent, toolSignature, toolContent } = getSignatureContext(signatureSessionId, actualModelName, hasTools);

    request.contents.forEach(content => {
      if (content.role === 'model' && content.parts && Array.isArray(content.parts)) {
        processModelThoughts(content, reasoningSignature, reasoningContent, toolSignature, toolContent, enableThinking, actualModelName);
      }
    });
  }
