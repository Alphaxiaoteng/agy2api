...
function handleClaudeAssistantMessage(message, antigravityMessages, enableThinking, actualModelName, sessionId, hasTools) {
  ...
  let thinkingContent = '';
  const toolCalls = [];
  let messageSignature = msgSignature || null;
  ...
  if (typeof content === 'string') {
    ...
    for (const item of content) {
      if (item.type === 'text') {
        ...
        if (!messageSignature && itemSig) messageSignature = itemSig;
      } else if (item.type === 'tool_use') {
        const safeName = processToolName(item.name, sessionId, actualModelName);
        const itemSig = item.signature || item.thoughtSignature || item.thought_signature || item.extra_content?.thought_signature;
        const signature = enableThinking
          ? (itemSig || toolSignature || messageSignature || reasoningSignature || getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE)
          : null;
        toolCalls.push(createFunctionCallPart(item.id, safeName, JSON.stringify(item.input || {}), (item.signature || item.thoughtSignature || item.thought_signature || (item.extra_content && item.extra_content.thought_signature) || signature || (isEnableThinking(actualModelName) ? (getToolSignatureForModel(actualModelName) || GEMINI_TOOL_SIGNATURE) : null)), actualModelName));
      }
    }
  }
...
export function generateClaudeRequestBody(claudeMessages, modelName, parameters, claudeTools, systemPrompt, token) {
  ...
  const tools = convertClaudeToolsToAntigravity(claudeTools, signatureSessionId, actualModelName);
  const hasTools = tools && tools.length > 0;
  const requestBody = buildRequestBody({
    contents: claudeMessageToAntigravity(claudeMessages, enableThinking, actualModelName, signatureSessionId, hasTools),
    tools: tools,
