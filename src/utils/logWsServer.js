...
import { filterLogEntries } from './logFilter.js';
...
class LogWebSocketServer {
    ...
    initialize(server) {
        if (!server) {
            throw new Error('HTTP server instance is required to initialize LogWebSocketServer');
        }
        ...
        this.wss.on('connection', (ws, req) => {
            ...
            // 发送最近的��心日志历史（失败/告警）
            const recentLogs = this.logStore.filter((log) => log.level === 'warn' || log.level === 'error').slice(-50);
    ...
    getLogs(options = {}) {
        ...
        filtered = filterLogEntries(filtered, { level, search });
