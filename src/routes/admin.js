...
import { refreshTokenQuotaCache } from '../api/client.js';
...
const localOAuthFlow = new LocalOAuthFlow({ oauthManager, tokenManager, logger });
...
router.post('/login', async (req, res) => {
  ...
  if (isLocal) {
    ...
    logger.info(`管理员本地自动登录成功 IP: ${clientIP}`);
...
// 手动触发周额度重置自动激活扫描
router.post('/tokens/activate-weekly', cookieAuthMiddleware, async (req, res) => {
  try {
    logger.info('手动触发周额度重置自动激活扫描');
    await weeklyActivationManager.checkAndActivateAll();
    res.json({ success: true, message: '周额度激活扫描完成', data: weeklyActivationManager.getStatus() });
  } catch (error) {
    logger.error('周额度激活扫描失败:', error.message);
    res.status(500).json({ success: false, message: error.message });
  }
});

// 查询周额度自动激活状态与记录
router.get('/tokens/activate-weekly/status', cookieAuthMiddleware, (req, res) => {
  res.json({ success: true, data: weeklyActivationManager.getStatus() });
});
...
function getSafeTokenIdentifier(token, tokenId) {
  if (token.email) return { email: token.email };
  return { tokenId };
}

export async function initializeImportedTokens(tokens, manager = tokenManager) {
  const results = [];

  for (const token of tokens) {
    const tokenId = await manager.getTokenId(token);
    const result = {
      ...getSafeTokenIdentifier(token, tokenId),
      success: false,
      refreshed: false,
    ...
    try {
      if (!tokenId) throw new Error('无法生成Token ID');

      await manager.refreshTokenById(tokenId);
      result.refreshed = true;

      if (!token.projectId) {
        await manager.fetchProjectIdForToken(tokenId);
        result.projectIdFetched = true;
      }

      await manager.refreshSubscriptionAndCreditsById(tokenId);
      result.subscriptionRefreshed = true;
      result.success = true;
    } catch (error) {
...
router.post('/tokens/import', cookieAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    await tokenManager.reload();
    const initialization = await initializeImportedTokens(parsedTokens);
...
router.post('/oauth/start', cookieAuthMiddleware, async (req, res) => {
  try {
    const flow = await localOAuthFlow.start(req.body?.mode || 'antigravity');
    res.json({ success: true, data: flow });
  } catch (error) {
    logger.error('启动OAuth失败:', error.message);
    res.status(500).json({ success: false, message: '无法启动OAuth流程' });
  }
});

router.get('/oauth/status/:id', cookieAuthMiddleware, (req, res) => {
  res.json({ success: true, data: localOAuthFlow.getStatus(req.params.id) });
});
...
router.put('/rotation', cookieAuthMiddleware, (req, res) => {
  try {
    const { strategy, requestCount, priorityAccountIds } = req.body;
    ...
    // 更新内存中的配置
    tokenManager.updateRotationConfig(strategy, requestCount, priorityAccountIds);
    ...
    if (priorityAccountIds !== undefined) {
      currentConfig.rotation.priorityAccountIds = normalizePriorityAccountIds(priorityAccountIds);
    }
...
router.get('/tokens/:tokenId/quotas', cookieAuthMiddleware, async (req, res) => {
  try {
    ...
    if (isDisabled) {
      ...
      if (!quotaData) {
        // 缓存未命中或强制刷新，从API获取
        await refreshTokenQuotaCache(tokenId, tokenData);
        // 从缓存中获取完整数据（包含 requestCounts），而不是构造不完整的对象
        quotaData = quotaManager.getQuota(tokenId) || { lastUpdated: Date.now(), models: {}, requestCounts: {} };
      }
    }
    ...
    Object.entries(quotaData.models || {}).forEach(([modelId, quota]) => {
        modelsWithBeijingTime[modelId] = {
          remaining: quota.r === null || quota.r === undefined ? null : quota.r,
          known: quota.known === true && quota.r !== null && quota.r !== undefined,
    ...
    res.json({
      ...
      data: {
        ...
        fetchedAt: quotaData.lastUpdated,
        source: forceRefresh ? 'upstream' : 'cache',
        models: modelsWithBeijingTime,
        requestCounts, // 返回请求计数供前端计算预估
        windows: quotaManager.formatQuotaWindows(quotaData),
...
// ==================== OpenCode 模型目录（本地 API 管理汇总） ====================
router.get('/catalog', cookieAuthMiddleware, async (req, res) => {
  try {
    const probe = req.query.probe !== '0';
    const data = await buildModelCatalog({ probe });
    res.json({ success: true, data });
  } catch (error) {
    logger.error('读取模型目录失败:', error.message);
