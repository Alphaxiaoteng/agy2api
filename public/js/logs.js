...
let logsState = {
    ...
    stats: { total: 0, info: 0, warn: 0, error: 0, request: 0, debug: 0, core: 0 },
    // WebSocket 相关
    ws: null,
    wsConnected: false,
...
async function clearLogs() {
    ...
    try {
        ...
        if (data.success) {
            ...
            logsState.logs = [];
            logsState.total = 0;
            logsState.stats = { total: 0, info: 0, warn: 0, error: 0, request: 0, debug: 0, core: 0 };
            renderLogs();
            renderLogStats();
        } else {
...
function renderLogStats() {
    ...
    statsContainer.innerHTML = `
        <div class="log-stat-item clickable ${currentLevel === 'core' ? 'active' : ''}" onclick="filterLogLevel('core')">
            <span class="log-stat-num">${(logsState.stats.core ?? ((logsState.stats.warn || 0) + (logsState.stats.error || 0)))}</span>
            <span class="log-stat-label">核心</span>
        </div>
        <div class="log-stat-item warn clickable ${currentLevel === 'warn' ? 'active' : ''}" onclick="filterLogLevel('warn')">
            <span class="log-stat-num">${logsState.stats.warn || 0}</span>
            <span class="log-stat-label">失败/警告</span>
        </div>
        <div class="log-stat-item error clickable ${currentLevel === 'error' ? 'active' : ''}" onclick="filterLogLevel('error')">
            <span class="log-stat-num">${logsState.stats.error || 0}</span>
            <span class="log-stat-label">错误</span>
        </div>
        <div class="log-stat-item clickable ${currentLevel === 'all' ? 'active' : ''}" onclick="filterLogLevel('all')">
            <span class="log-stat-num">${logsState.stats.total || 0}</span>
...
function logMatchesFilter(log) {
    if (logsState.currentLevel === 'all') return true;
    if (logsState.currentLevel === 'core') return log.level === 'warn' || log.level === 'error';
    return log.level === logsState.currentLevel;
}
...
function addNewLog(log) {
    ...
    if (!isSeparatorLine(log.message)) {
        ...
        if (log.level === 'warn' || log.level === 'error') {
            logsState.stats.core = (logsState.stats.core || 0) + 1;
        }
        renderLogStats();
    }
...
function updateStats() {
    const stats = { total: 0, info: 0, warn: 0, error: 0, request: 0, debug: 0, core: 0 };
    for (const log of logsState.logs) {
        ...
        if (log.level === 'warn' || log.level === 'error') stats.core++;
    }
