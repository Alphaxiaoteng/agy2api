...
function renderTokens(tokens) {
    ...
    tokenList.innerHTML = filteredTokens.map((token, index) => {
        ...
        const validationStatus = token.statusReason === 'VALIDATION_REQUIRED';
        ...
        <div class="token-card ${!token.enable ? 'disabled' : ''} ${isRefreshing ? 'refreshing' : ''} ${skipAnimation ? 'no-animation' : ''}" id="card-${escapeHtml(cardId)}">
            <div class="token-header">
                <div class="token-header-left">
                    <span class="status ${token.enable ? 'enabled' : 'disabled'}">
                        ${validationStatus ? '需验证' : (token.enable ? '启用' : '禁用')}
                    </span>
                ...
                <div class="token-header-right">
                    <button class="btn-icon" onclick="showTokenDetail('${safeTokenId}')" title="编辑">✎</button>
                    <span class="token-id">#${tokenNumber}</span>
                </div>
            </div>
            <div class="token-email-row editable" onclick="editField(event, '${safeTokenId}', 'email', '${safeEmailJs}')" title="点击编辑邮箱">
                <span class="token-email info-value" data-email="${safeEmail}">${safeEmail || '未设置邮箱'}</span>
            </div>
            ${validationStatus ? `<div class="info-row validation-required"><span class="info-value">需 Google 验证</span>${token.validationUrl ? `<button class="btn btn-xs btn-warning" onclick="openValidationUrl(event, '${escapeJs(token.validationUrl)}')">打开验证</button>` : ''}</div>` : ''}
            <div class="token-quota-inline" id="quota-inline-${escapeHtml(cardId)}">
                <div class="quota-inline-header" onclick="toggleQuotaExpand('${escapeJs(cardId)}', '${safeTokenId}')">
                    <span class="quota-inline-summary" id="quota-summary-${escapeHtml(cardId)}">加载额度...</span>
            ...
            <details class="token-meta sensitive-row">
                <summary>账号详情</summary>
                <div class="token-info">
                    <div class="info-row editable" onclick="editField(event, '${safeTokenId}', 'projectId', '${safeProjectIdJs}')" title="点击编辑">
                        <span class="info-label">PID</span>
                        <span class="info-value">${safeProjectId || '点击设置'}</span>
                        <button class="btn btn-xs btn-info fetch-project-btn" onclick="fetchProjectId(event, '${safeTokenId}')" title="从API获取Project ID">获取</button>
                    </div>
                    <div class="token-id-row" title="Token ID: ${escapeHtml(tokenId)}">
                        <span class="token-id-label">ID</span>
            ...
            <div class="token-actions">
                ...
                <button class="btn btn-danger btn-xs" onclick="deleteToken('${safeTokenId}')" title="删除">删除</button>
            </div>
        </div>
    `}).join('');
...
async function loadPoolRotationHint() {
    const el = document.getElementById('poolRotationHint');
    if (!el) return;
    try {
        ...
        const names = {
            round_robin: '周额度优先 · 5h 均衡 · 新会话轮询',
            quota_exhausted: '粘性：用尽再切',
            request_count: `每号 ${data.data.requestCount || 10} 次后切换`
        };
        el.textContent = names[data.data.strategy] || data.data.strategy;
        el.hidden = false;
    } catch (error) {
        el.hidden = true;
    }
...
function openValidationUrl(event, url) {
    event.stopPropagation();
    if (typeof url === 'string' && /^https:\/\/accounts\.google\.com\//.test(url)) {
