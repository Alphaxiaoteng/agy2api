...
const mockToken = { sessionId: 'test-session', projectId: 'test-project' };
...
console.log(`✓ 第2条 (model with text and tool calls): ${contents[1]?.role === 'model' && contents[1]?.parts?.length === 3 ? '✓' : '✗'}`);
console.log(`✓ 第3条 (user with tool responses): ${contents[2]?.role === 'user' && contents[2]?.parts?.length === 2 ? '✓' : '✗'}`);
