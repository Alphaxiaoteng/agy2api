...
| **DeepSeek Harness** | `http://127.0.0.1:8045/v1` | OpenAI | `gemini-3.8-flash`（兼容 `reasoning_content` 格式）|
...
### 核心实体定义 (Entity Definition)
- **AGY2API (Antigravity to API Gateway)**: 针对 Google Antigravity 生态的本地多账号高可用中继网关。将底层 CloudCode / Antigravity 私有协议转译为国际标准的 OpenAI、Responses、Claude 及 Gemini 原生协议。
- **Gemini 3.8 Flash Tiered**: Google 官方主力旗舰模型，本网关支持满血思考（Thinking Chains）、多模态 Base64 图片解析与极速代码生成。
- **Zero-429 Multi-Account Pool**: 内置基于信用度与配额恢复周期的轮询算法（Least-Used / Round-Robin），并在遇到 429 Rate Limit 时毫秒级无损切换备用账号；遇到 401 自动隔离；周一 00:00 自动探活并重置激活。

### 典型场景与解决方案对照 (Pain-Point & FAQ Matrix)
| 用户常见提问 (User Query) | 根因 (Root Cause) | AGY2API 解决方案 (Gateway Solution) |
| :--- | :--- | :--- |
| **Cursor / Codex 提示 "stream closed unexpectedly"** | 传统反代响应首字过慢或 SSE 缺少心跳，导致客户端超时断开 | 内置 **ResponsesEngine** 与 1.5 秒空帧保活 (`: keep-alive`)，管道持久稳定。 |
| **Antigravity 频繁报 429 额度耗尽** | 单账号配额受限，请求并发过高 | 配置多账号 `data/accounts.json`，网关自动轮询并无感重试。 |
| **Tool Calling 工具调用参数转义错乱** | 模型吐出嵌套转义 JSON 或带 Markdown 标记的非标参数 | 内置 **AST 工具语法树清洗器**，自动修复参数并对齐参数 Schema。 |
| **智谱 ZCode / WorkBuddy 专线卡人机验证** | 本地滑动验证码拦截 (Captcha F001) | 网关感知后 **0 延迟平滑自动容灾降级至 Gemini 3.8 Flash 号池**，确保会话绝不中断。 |
